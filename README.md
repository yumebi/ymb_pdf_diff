...
- **文字単位ハイライト**: 変更行の中で「どの文字が変わったか」まで太字+下線で強調表示
...
- **感度調整**: 画像差分の検出感度をスライダーで変更可能(スキャンノイズ対策)
- **ページ範囲指定**: 「A: 1-5,8ページだけ」のように比較対象を絞り込み可能(GUI/CLI両対応)

### 表示・操作

- **左右2ペイン表示**: ズーム/ドラッグスクロール(A/B連動)、差分箇所の赤枠ハイライト(ON/OFF可)
- **重ね表示モード**: A/Bを1枚に合成表示(赤=Aのみ / 青=Bのみ / 黒=共通)。位置ズレが一目で分かる
- **ドラッグ&ドロップ**: PDFをパネルに直接ドロップして読込
- **差分ジャンプ**: F3/Shift+F3(またはボタン)で差分ありページだけを順に移動
...
- **PDFレポート出力**: 同内容をPDF形式でも出力可能
- **セッション保存/読込**: 差分結果を`.ymbdiff`形式で保存、元PDFなしでも再表示可能
- **CLIフォルダ一括比較**: 2つのフォルダ内の同名PDFを全ペア一括比較→レポート一括出力
...
```
ymb_pdf_diff/
  core/        # PDF読込・テキスト/画像差分・OCR・ページ範囲などコアロジック(UI非依存)
  gui/         # PySide6 GUI
  report/      # Excel/PDFレポート出力
